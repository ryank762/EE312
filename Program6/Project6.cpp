...
int minRec2(int x[], int n) {
	if (n == 1) {
		...
		int mid = (n+1)/2;
		return minRec2(x, mid) < minRec2(&x[mid], n-mid) ? minRec2(x,mid) : minRec2(&x[mid], n-mid);
	}
}
...
double sqrtIt(double x, double low_guess, double high_guess) {
	...
	int j = 0, n = 1;
	double sqrt = 0;
	while (low_guess < high_guess && j < 16) {
		for (i = 0; sqrt*sqrt < x ; i++) {
			sqrt += (double) i/n;
		}
		high_guess = sqrt;
		sqrt -= (double) 1/n;
	 ...
	 */
	double sqrt;
	while ((high_guess - low_guess) > (double) 1/100000000000000) {
		sqrt = (low_guess + high_guess) / 2;
		if (sqrt * sqrt < x) {
			low_guess = sqrt;
		} else if (sqrt * sqrt > x) {
			high_guess = sqrt;
...
double sqrtRec(double x, double low_guess, double high_guess) {
	if ((high_guess - low_guess) < (double) 1/100000000000000) {
		return high_guess;
	} else {
...
int strCompare2(char* str1, char* str2) {
	if (!str1[0] && !str2[0]) {
		return 0;
	} else if (whatLetter(str1[0]) < 0) {
		return strCompare2(str1 + 1, str2);
	} else if (whatLetter(str2[0]) < 0) {
...
int solveMazeRec(int row, int col) {
	...
	// recursive case
	if (row > 0 && maze[row-1][col] == 0) {                 // try going up
		if (solveMazeRec(row-1, col) == 1) {
			return 1;
		}

	}
	if (col < MATRIX_SIZE - 1 && maze[row][col+1] == 0) {   // try going right
		if (solveMazeRec(row, col+1) == 1) {
			return 1;
		}

	}
	if (row < MATRIX_SIZE - 1 && maze[row+1][col] == 0) {   // try going down
		if (solveMazeRec(row+1, col) == 1) {
			return 1;
		}

	}
	if (col > 0 && maze[row][col-1] == 0) {                 // try going left
		if (solveMazeRec(row, col-1) == 1) {
			return 1;
		}
...
Martian change(int cents) {
	...
	// recursive case
	if (cents > 12) {
		result1 = change(cents - 12);
		result1.dodeks++;
		result2 = change(cents - 5);
		result2.nicks++;
		result3 = change(cents - 1);
		result3.pennies++;
		sum1 = result1.dodeks + result1.nicks + result1.pennies;
		sum2 = result2.dodeks + result2.nicks + result2.pennies;
		sum3 = result3.dodeks + result3.nicks + result3.pennies;
...
Martian change(int cents, int nick_val, int dodek_val) {
	...
	// recursive case
	if (cents > dodek_val) {
		result1 = change(cents - dodek_val, nick_val, dodek_val);
		result1.dodeks++;
		result2 = change(cents - nick_val, nick_val, dodek_val);
		result2.nicks++;
		result3 = change(cents - 1, nick_val, dodek_val);
		result3.pennies++;
		sum1 = result1.dodeks + result1.nicks + result1.pennies;
		sum2 = result2.dodeks + result2.nicks + result2.pennies;
		sum3 = result3.dodeks + result3.nicks + result3.pennies;
